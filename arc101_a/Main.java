...
public class Main {
  ...
  static int solve(int[] x, int K) {
    ...
    int[] leftDistances =
        IntStream.range(0, middleIndex).map(i -> -x[middleIndex_ - 1 - i]).toArray();
    int[] rightDistances =
        IntStream.range(0, x.length - middleIndex).map(i -> x[middleIndex_ + i]).toArray();

    return Math.min(
        computeMinTime(K, leftDistances, rightDistances),
