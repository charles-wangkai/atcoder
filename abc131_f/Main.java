...
public class Main {
  ...
  static long solve(int[] x, int[] y) {
    ...
    for (int i = 0; i < N; ++i) {
      xValueToIndices.putIfAbsent(x[i], new ArrayList<>());
      xValueToIndices.get(x[i]).add(i);

      yValueToIndices.putIfAbsent(y[i], new ArrayList<>());
    ...
    for (List<Integer> indices :
        Stream.concat(xValueToIndices.values().stream(), yValueToIndices.values().stream())
            .toList()) {
