...
public class Main {
  ...
  static int solve(int[] a, int[] b, int[] c, int Ma, int Mb) {
    ...
    for (int i = 0; i < a.length; ++i) {
      ...
      for (State state : stateToCost.keySet()) {
        ...
        nextStateToCost.put(
            nextState,
            Math.min(
                nextStateToCost.getOrDefault(nextState, Integer.MAX_VALUE),
    ...
    return stateToCost.keySet().stream()
        .filter(state -> state.sumA() != 0 && state.sumA() * Mb == state.sumB() * Ma)
        .mapToInt(stateToCost::get)
        .min()
        .orElse(-1);
  }
}

record State(int sumA, int sumB) {}
