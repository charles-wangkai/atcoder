...
public class Main {
            ...
            Math.max(
                ...
                Math.max(
                    ...
                    computeRectangleSum(prefixSums, 0, minY, minX - 1, minY + K - 1)
                        ...
                        + computeRectangleSum(
                            prefixSums, minX, minY + K, minX + K - 1, 2 * K - 1)));
      }
    }
  ...
  static int computeRectangleSum(
      int[][] prefixSums, int lowerX, int lowerY, int upperX, int upperY) {
    if (lowerX > upperX || lowerY > upperY) {
      return 0;
    }

    return prefixSums[upperX + 1][upperY + 1]
        - prefixSums[lowerX][upperY + 1]
        - prefixSums[upperX + 1][lowerY]
