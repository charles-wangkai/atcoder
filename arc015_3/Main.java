...
public class Main {
  ...
  static String solve(String[] large, int[] m, String[] small) {
    List<String> units =
        Stream.concat(Arrays.stream(large), Arrays.stream(small)).distinct().toList();
    Map<String, Integer> unitToIndex =
        IntStream.range(0, units.size())
            .boxed()
            .collect(Collectors.toMap(units::get, Function.identity()));
    ...
    for (int i = 0; i < unitNum; ++i) {
      for (int j = 0; j < unitNum; ++j) {
        if (conversions[i][j] > maxConversion) {
          ...
          result = "1%s=%d%s".formatted(units.get(i), Math.round(conversions[i][j]), units.get(j));
        }
      }
    }
