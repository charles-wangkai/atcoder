...
public class Main {
  ...
  static String buildAssignment(String s, String prefix) {
    ...
    return (another(s, result, result.length() - 1, result.charAt(result.length() - 2))
                == result.charAt(0)
            && another(s, result, 0, result.charAt(1)) == result.charAt(result.length() - 1))
        ? result.toString()
        : null;
  }
  ...
  static char another(String s, StringBuilder assignment, int index, char neighbor) {
    if (assignment.charAt(index) == 'S') {
      return (s.charAt(index) == 'o') ? neighbor : invert(neighbor);
    }

    return (s.charAt(index) == 'o') ? invert(neighbor) : neighbor;
  }
