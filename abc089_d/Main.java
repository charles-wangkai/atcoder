...
public class Main {
    ...
    return IntStream.range(0, L.length)
        .map(
            i -> {
              ...
              return prefixSumLists[m].get(indexR) - prefixSumLists[m].get(indexL);
            })
  ...
  static int computeDistance(Point[] points, int index1, int index2) {
    ...
    return Math.abs(points[index1].r() - points[index2].r())
        + Math.abs(points[index1].c() - points[index2].c());
  }
}
