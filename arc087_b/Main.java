    ...
    for (char command : s.toCharArray()) {
      if (command == 'F') {
        if (xOrY) {
          xStates =
              xStates.stream()
                  .map(xState -> new State(xState.value() + xState.offset(), xState.offset()))
                  .collect(Collectors.toSet());
        } else {
          yStates =
              yStates.stream()
                  .map(yState -> new State(yState.value() + yState.offset(), yState.offset()))
                  .collect(Collectors.toSet());
        }
      } else {
        xOrY ^= true;
        ...
        if (xOrY) {
          xStates.addAll(
              xStates.stream()
                  .map(xState -> new State(xState.value(), -xState.offset()))
                  .collect(Collectors.toList()));
        } else {
          yStates.addAll(
              yStates.stream()
                  .map(yState -> new State(yState.value(), -yState.offset()))
                  .collect(Collectors.toList()));
        }
      }
    }
...
record State(int value, int offset) {}
