...
public class Main {
  ...
  static long search(long[] totalNums, long[] pattyNums, int level, long x) {
    ...
    if (x <= totalNums[level - 1] + 1) {
      return search(totalNums, pattyNums, level - 1, x - 1);
    }
    if (x == totalNums[level - 1] + 2) {
      return pattyNums[level - 1] + 1;
    }
    if (x <= totalNums[level - 1] * 2 + 2) {
      return pattyNums[level - 1]
          + 1
          + search(totalNums, pattyNums, level - 1, x - (totalNums[level - 1] + 2));
    }
