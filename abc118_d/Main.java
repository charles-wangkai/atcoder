...
public class Main {
  ...
  static BigInteger solve(int N, int[] A) {
    ...
    for (int digit : A) {
      stickNumToDigit.put(
          STICK_NUMS[digit], Math.max(stickNumToDigit.getOrDefault(STICK_NUMS[digit], 0), digit));
    }

    int[] stickNums =
        stickNumToDigit.keySet().stream().sorted().mapToInt(Integer::intValue).toArray();
  ...
  static void search(
      Map<Integer, Integer> stickNumToDigit,
      int[] stickNums,
      int[] lengths,
      int index,
      int totalLength,
