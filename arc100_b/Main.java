...
public class Main {
  ...
  static long solve(int[] A) {
    ...
    for (int middleIndex = 1; middleIndex + 2 < A.length; ++middleIndex) {
      ...
      List<long[]> rightNearestPairs =
          findNearestPairs(prefixSums, prefixSums[middleIndex], middleIndex + 1, A.length - 1);

      for (long[] leftNearestPair : leftNearestPairs) {
        for (long[] rightNearestPair : rightNearestPairs) {
          long[] sorted =
              LongStream.concat(Arrays.stream(leftNearestPair), Arrays.stream(rightNearestPair))
                  .sorted()
                  .toArray();
  ...
  static List<long[]> findNearestPairs(
      long[] prefixSums, long basePrefixSum, int beginIndex, int endIndex) {
